...
def filter_dataset(dataset, data_filename,
                   percentile, min_nrof_images_per_class):
...
def train(args, sess, epoch, image_list, label_list,
          index_dequeue_op, enqueue_op, image_paths_placeholder,
          labels_placeholder, learning_rate_placeholder,
          phase_train_placeholder, batch_size_placeholder,
...
def evaluate(sess, enqueue_op, image_paths_placeholder,
             labels_placeholder, phase_train_placeholder,
             batch_size_placeholder,
             embeddings, labels, image_paths,
             actual_issame, batch_size, nrof_folds,
    ...
    assert nrof_images % batch_size == 0, 'The number of LFW images must be an integer multiple of the LFW batch size'  # noqa
    ...
    ) is True, 'Wrong labels used for evaluation, possibly caused by training examples left in the input pipeline'  # noqa
...
def save_variables_and_metagraph(sess, saver, summary_writer,
                                 model_dir, model_name, step):
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        ...
        help='Upper bound on the amount of GPU memory that will be used by the process.', default=1.0  # noqa
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--model_def', type=str,
        help='Model definition. Points to a module containing the definition of the inference graph.',  # noqa
    ...
    parser.add_argument(
        '--random_crop',
        help='Performs random cropping of training images. If false, the center image_size pixels from the training images are used. If the size of the images in the data directory is equal to image_size no cropping is performed', action='store_true'  # noqa
        )
    parser.add_argument(
        '--random_flip',
    ...
    parser.add_argument(
        ...
        help='Initial learning rate. If set to a negative value a learning rate schedule can be specified in the file "learning_rate_schedule.txt"', default=0.1  # noqa
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--moving_average_decay', type=float,
    ...
    parser.add_argument(
        '--nrof_preprocess_threads', type=int,
        help='Number of preprocessing (data loading and augmentation) threads.', default=4  # noqa
    )
    parser.add_argument(
        '--log_histograms',
    ...
    parser.add_argument(
        ...
        help='File containing the learning rate schedule that is used when learning_rate is set to to -1.',  # noqa
        default='data/learning_rate_schedule.txt'
    )
    parser.add_argument(
        '--filter_filename', type=str,
        help='File containing image data used for dataset filtering',
        default=''
    )
    parser.add_argument(
        '--filter_percentile', type=float,
        help='Keep only the percentile images closed to its class center',
        default=100.0
    )
    parser.add_argument(
        '--filter_min_nrof_images_per_class', type=int,
    ...
    parser.add_argument(
        ...
        help='Number of folds to use for cross validation. Mainly used for testing.', default=10  # noqa
    )
